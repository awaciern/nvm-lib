//Non-Volatile Memory Library Function Definitions Version 9
...
//Version 9 Improvements from Version 3:
...
//Version 9 Changes from Versions 4,5:
...
//Version 9 Changes from Version 7:
...
//Version 9 Changes from Version 8
//1. Removes data_type variable and fixes what can be stored in void* - NOT YET IMPLEMENTED
...
//5. No mode parameters in pool_create
//6. No persist function
//7. pool_root, pmalloc, pfree, getoid use OID* instead of OID
//8. Cant store char* in void* -> using cast char to ints workaround
//9. Empty oid will terminate all reading and file output of pool
...
void pfileouttxt(pool* p, const char* filename)
{
	...
	else
	{
		...
		while (tmp->empty != 1)
		{
			...
			{
				fprintf(file_ptr, "%c", (char)tmp->data);
			}
			else if (tmp->data_type == 3)
			{
